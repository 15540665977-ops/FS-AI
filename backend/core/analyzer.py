...
━━━ 输出结构要求（必须严格遵守）━━━
每次分析报告必须按以下固定顺序输出，不得颠倒：

第一部分：【综合结论摘要】（必须是报告开头，3-5行）
格式：
> **综合结论**
> - **材料鉴定：** [材料名称及类型]（置信度：确认/疑似/不确定）
> - **主要发现：** [最关键的1-2个发现，如异常、成分差异等]
> - **失效/风险推断：** [若无则填"未见明显失效迹象"]
> - **建议行动：** [下一步建议测试或处理方向]
> ---

第二部分：详细分析过程（峰位逐一核对、材料鉴定依据、失效推断等）
...
谱图曲线分析规范（必须遵守）：
6. 【核心要求】提示词中的「图像曲线自动提取数据 v2」列出了所有检测到的峰（含图中未标注的峰）。
   必须逐一核对该列表中的每个峰，给出归属与解读。不得只分析图中已有文字标签的峰。
7. 未标注峰的分析方法：根据波数范围推断可能的官能团归属，说明为何图中未标注（如弱峰、肩峰、添加剂干扰）
8. 对每个峰描述：峰形（窄/宽/不对称/有肩峰）、相对强度（对应提取数据中的很强/强/中/弱）
9. 对于宽峰或重叠峰，明确指出可能存在多个组分叠加
10. 自动提取峰位精度约 ±20–50 cm⁻¹；以图中已标注数值为准，提取数据用于发现未标注峰

禁止使用模糊表述，如"可能"、"大约"、"类似"——用置信度标注代替。
禁止仅描述图中已标注的峰而忽略提取数据中的未标注峰。
禁止把综合结论放在报告末尾——必须作为第一部分输出。"""
...
class AnalysisOrchestrator:
    ...
    def _build_prompt_text(
        ...
        # v2：提取数据放最前面，让 Claude 优先建立"有哪些峰"的完整认知
        ...
        if analysis_type == "failure":
            ...
            parts.append(
                "这些图像来自【同一材料样品】的多种检测（可能包含 FTIR 红外光谱、DSC 差示扫描量热、"
                "TGA 热重分析中的几种）。\n\n"
                "请按以下步骤进行联合分析：\n"
